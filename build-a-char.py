...
def optimize_equipment(stat_to_optimize, debug=False):
    optimized_equipment = {}
    ...
    for loc in wearLocs:
        ...
        for key, value in eqDict.items():
            ...
            if classes and genres:
                if selected_class.lower() not in classes[0].lower() and selected_genre.lower() not in genres[0].lower():
                    class_allowed = False
                    genre_allowed = False
            elif classes:
            ...
            if not class_allowed or not genre_allowed:
